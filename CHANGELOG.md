...
- 🔄 **Graceful Shutdown for WS/SSE**
  - `hub.Shutdown(ctx)` — корректное закрытие всех соединений
  - WebSocket отправляет `CloseGoingAway` перед закрытием
  - SSE отправляет `shutdown` event клиентам

- ⏱️ **Read/Write таймауты**
  - `WSConfig.ReadTimeout` — таймаут чтения для WebSocket
  - `WSConfig.HandshakeTimeout` — таймаут handshake
  - `SSEConfig.WriteTimeout` — таймаут записи для SSE

- 🔁 **Поддержка переподключения клиентов**
  - `SSEWriter.LastEventID()` — получение Last-Event-ID
  - `SSEWriter.IsReconnect()` — проверка реконнекта
  - `examples/reconnect.js` — JavaScript хелперы для авто-реконнекта

### Fixed

- 🧹 Removed unused `isRunning()` function from BaseHub
